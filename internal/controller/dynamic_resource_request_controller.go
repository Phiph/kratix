...
func (r *DynamicResourceRequestController) updateResourceBinding(ctx context.Context, logger logr.Logger, rr *unstructured.Unstructured, promise *v1alpha1.Promise) error {
	...
	if op == "created" {
		r.EventRecorder.Eventf(rr, nil, v1.EventTypeNormal, "BindingCreated", "BindingCreated", "%s", fmt.Sprintf("Binding %s created for promise %s version %s",
...
func (r *DynamicResourceRequestController) reconcileSuspendedWorkflow(
	...
	if !nextRetryAtTime.IsZero() {
		if nextRetryAtTime.Before(time.Now()) {
			logging.Info(logger, "the recorded NextRetryAt time has been reached, "+
				"removing the workflow-suspended label", "nextRetryAtTime", nextRetryAtTime)
			resourceLabels := rr.GetLabels()
			delete(resourceLabels, v1alpha1.WorkflowSuspendedLabel)
			rr.SetLabels(resourceLabels)
			return true, nil, r.Client.Update(ctx, rr)
		}
		requeueAfterDuration := time.Until(nextRetryAtTime)
...
func (r *DynamicResourceRequestController) updatePromiseVersionStatus(logger logr.Logger, rr *unstructured.Unstructured, bindingVersion string, promiseRevision *v1alpha1.PromiseRevision) bool {
	...
	if currentVersion != promiseRevision.Spec.Version {
		...
		r.EventRecorder.Eventf(rr, nil, v1.EventTypeNormal, "ReconcileSucceeded", "ReconcileSucceeded", "Resource request reconciled with promise %s version %s",
	...
	if currentBindingVersion != bindingVersion {
		...
		r.EventRecorder.Eventf(rr, nil, v1.EventTypeNormal, "ResourceBindingVersionUpdated", "ResourceBindingVersionUpdated", "Resource binding version updated to %s", bindingVersion)
		versionUpdated = true
	}
...
func (r *DynamicResourceRequestController) setPausedReconciliationStatusConditions(ctx context.Context, rr *unstructured.Unstructured, eventMsg string) error {
	...
	if reconciled == nil || reconciled.Status != "Unknown" || reconciled.Message != "Paused" {
		...
		r.EventRecorder.Eventf(rr, nil, v1.EventTypeWarning, pausedReconciliationReason, pausedReconciliationReason, "%s", eventMsg)
...
func (r *DynamicResourceRequestController) deleteResources(o opts, promise *v1alpha1.Promise, resourceRequest *unstructured.Unstructured) (ctrl.Result, error) {
	...
	if controllerutil.ContainsFinalizer(resourceRequest, resourceBindingFinalizer) {
		...
		// Fall through. deleteResourceBinding removes the finalizer iff its
		// cleanup is complete; if not yet complete, we requeue at the end.
	}
...
func getPromiseRevisionForReconcile(ctx context.Context, rr *unstructured.Unstructured, baseLogger logr.Logger, r *DynamicResourceRequestController, promise *v1alpha1.Promise) (*v1alpha1.PromiseRevision, *v1alpha1.ResourceBinding, error) {
	...
	if err != nil {
		...
		r.EventRecorder.Eventf(rr, nil, v1.EventTypeWarning, promiseRevisionLookupFailedReason, promiseRevisionLookupFailedReason, "%s", err.Error())
		return nil, nil, err
	}
...
func (r *DynamicResourceRequestController) getRRFinalizers() []string {
	return []string{workFinalizer, removeAllWorkflowJobsFinalizer, runDeleteWorkflowsFinalizer, resourceBindingFinalizer}
}
...
func nextRetryAtForResource(rr *unstructured.Unstructured) (time.Time, error) {
	...
	for _, pipeline := range pipelines {
		if pipeline["phase"] == v1alpha1.WorkflowPhaseSuspended {
			if v, ok := pipeline["nextRetryAt"].(string); ok && v != "" {
