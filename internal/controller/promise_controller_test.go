...
var (
	...
	breakerEventRecorder   *record.FakeRecorder
)
...
var _ = Describe("PromiseController", func() {
	BeforeEach(func() {
		...
		breakerEventRecorder = record.NewFakeRecorder(1024)
		// Route manager-fetched recorders (e.g. r.Manager.GetEventRecorderFor("PromiseController"))
		// through a FakeRecorder so tests can observe those events too.
		m.GetEventRecorderForReturns(breakerEventRecorder)
		reconciler = &controller.PromiseReconciler{
		...
		When("the promise is being deleted", func() {
			When("the Promise has no delete workflow", func() {
				...
				When("a Promise's restart-required annotation (rate limit) is updated", func() {
					It("emits a single RuntimeOptionsRestartRequired warning event and does not re-emit on subsequent reconciles", func() {
						...
						for {
							select {
							case <-breakerEventRecorder.Events:
								continue
							default:
							}
							break
					...
					drain:
						for {
							select {
							case e := <-breakerEventRecorder.Events:
			...
			When("the Promise has a delete workflow", func() {
				...
				When("the delete pipeline is actively suspended", func() {
					...
					It("does not remove the delete finalizer while suspended", func() {
						_, err := reconciler.Reconcile(ctx, ctrl.Request{NamespacedName: promiseName})
						Expect(err).NotTo(HaveOccurred())
				...
				When("the delete pipeline is suspended with a retryAfter interval that has elapsed", func() {
					...
					It("removes the workflow-suspended label so the delete pipeline can be re-triggered", func() {
						...
						Expect(fakeK8sClient.Get(ctx, promiseName, promise)).To(Succeed())
						Expect(promise.GetLabels()).NotTo(HaveKey(v1alpha1.WorkflowSuspendedLabel))
					})
				})
