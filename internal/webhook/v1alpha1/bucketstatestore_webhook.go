...
func (v *BucketStateStoreCustomValidator) ValidateCreate(ctx context.Context, obj *v1alpha1.BucketStateStore) (admission.Warnings, error) {
	bucketstatestorelog.Info("Validation for BucketStateStore upon creation", "name", obj.GetName())

	if err := obj.ValidateSecretRef(); err != nil {
		return nil, err
	}
